"""
Bestmate Engine — One-time data seed script
Run locally after placing all Excel files in backend/scripts/data/
...
# ── File paths (place xlsx files in backend/scripts/data/) ────
DATA_DIR = os.path.join(os.path.dirname(__file__), "data")

FILES = {
    "rm_formulas":  os.path.join(DATA_DIR, "RM_Consolidated_SIP_Formulas.xlsx"),
    "tracker":      os.path.join(DATA_DIR, "Tracker_For_Backoffice_xlsb.xlsx"),
    "bounce":       os.path.join(DATA_DIR, "SIP_BOUNCE_DATA.xlsx"),
    "targets":      os.path.join(DATA_DIR, "Target_Framework____Team_View.xlsx"),
}
...
def chunk_insert(table, records):
...
try:
    ...
    chunk_insert("relationship_managers", records)
...
try:
    ...
    chunk_insert("sip_records", records)
except Exception as e:
    print(f"  ✗ Skipped: {e}")

# ── 3. Client Tasks (RM→Ops link) ─────────────────────────────
...
try:
    ...
    chunk_insert("client_tasks", records)
except Exception as e:
    print(f"  ✗ Skipped: {e}")
...
try:
    ...
    chunk_insert("sip_bounce", records)
except Exception as e:
    print(f"  ✗ Skipped: {e}")
...
try:
    ...
    sb.table("ops_members").insert([
        {"name": n, "role": "Backoffice", "is_active": True} for n in ops_names
    ]).execute()
    print(f"  ✓ Inserted {len(ops_names)} ops members")
except Exception as e:
    print(f"  ✗ Skipped: {e}")

print("\n✅  All data seeded successfully!")
print("   Now visit https://bestmateengine.vercel.app to explore.")
